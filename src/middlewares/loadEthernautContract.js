...
export default store => next => action => {
  ...
  if(
    ...
    !state.levels.ethernautDeployAddress
  ) {
    // console.log(`UNABLE TO LOAD ETHERNAUT`)
    return next(action)
  }
  // console.log(`GETTING ETHERNAUT...`, state.levels.ethernautDeployAddress)
  ...
  Ethernaut.at(state.levels.ethernautDeployAddress)
    .then(instance => {
      ...
      if(state.levels.activeLevel)
        store.dispatch(actions.loadLevelInstance(state.levels.activeLevel, true))
    })
    .catch(() => {
