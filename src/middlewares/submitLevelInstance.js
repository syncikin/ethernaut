...
export default store => next => async action => {
  ...
  if(
    ...
    !state.contracts.levels[action.level.factoryContractDeployedAddress] ||
  ...
  let completed = await submitLevelInstance(
    state.contracts.ethernaut,
    action.level.factoryContractDeployedAddress,
    state.contracts.levels[action.level.factoryContractDeployedAddress].address,
    state.player.address,
    state.network.gasPrice
  )
  if(completed) {
