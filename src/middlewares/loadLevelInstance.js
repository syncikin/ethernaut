...
export default store => next => action => {
  ...
  if(action.instanceAddress) instanceAddress = action.instanceAddress
    ...
    const cache = state.player.emittedLevels[action.level.factoryContractDeployedAddress]
  ...
  if(!instanceAddress && !action.reuse) {
    ...
    const showErr = function(error) {
      console.error('̿@bad Unable to retrieve level instance! Please check gas amount and try again.', error || '')
    }

    // const estimate = await state.contracts.ethernaut.getLevelInstance.estimateGas(action.level.factoryContractDeployedAddress)
    const estimate = parseInt(action.level.instanceGas, 10) || 2000000
    const insertCoin = state.network.web3.toWei(parseInt(action.level.insertCoin, 10), 'ether')
    state.contracts.ethernaut.createLevelInstance(action.level.factoryContractDeployedAddress, {
      gas: estimate,
  ...
  const Instance = ethutil.getTruffleContract(
    require(`../../build/contracts/${action.level.instanceContractName}.json`),
    {
      from: state.player.address,
      gasPrice: state.network.gasPrice
    }
