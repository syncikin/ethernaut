...
export default store => next => action => {
  ...
  if(state.levels.activeLevel) {
    store.dispatch(actions.deactivateLevel(state.levels.activeLevel))
  }

  // Find level from deployed level address
  const activeLevel = _.find(
    state.levels.list,
    level => level.factoryContractDeployedAddress === action.address
  )
  ...
  window.level = activeLevel.factoryContractDeployedAddress;
  console.info(`=> Level address\n${activeLevel.factoryContractDeployedAddress}`)
